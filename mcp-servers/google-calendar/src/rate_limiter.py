...
このモジュールは、Google Calendar APIのレート制限を遵守するために
...
class RateLimiter:
    ...
    Google Calendar APIのレート制限を守るため、
    ...
    Args:
        tokens_per_second: 1秒あたりに補充されるトークン数（デフォルト: 1.5）
        capacity: バケットの最大容量（デフォルト: 10）

    Example:
        >>> limiter = RateLimiter(tokens_per_second=1.5, capacity=10)
        >>> async with limiter:
        ...     # Google Calendar APIリクエストを実行
        ...     response = await service.events().list(...).execute()
    """

    def __init__(self, tokens_per_second: float = 1.5, capacity: int = 10) -> None:
        """RateLimiterを初期化.
